...
(function () {
  ...
  const CONFIG = {
    ...
    proxyUrl:       (scriptTag.getAttribute("data-proxy-url") || window.location.origin).replace(/\/$/, ""),
  ...
  if (!CONFIG.checkfrontHost || !CONFIG.razorpayKey) {
    console.error("[Razorpay×Checkfront] Missing required config: data-checkfront-host, data-razorpay-key");
    return;
  }
